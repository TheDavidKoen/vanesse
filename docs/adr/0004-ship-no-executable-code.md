...
- Publishing credentials never enter the repository. The one stored credential, an Open VSX
  token, lives only on the approval-gated `release` environment, and the Marketplace takes a
  manual upload, see ADR 0006.
